    ...
    def __init__(self,
        ...
        if use_quantum:
            ...
            self.qfc = QuantumLinear(in_features=feature_dims,
                                     ...
                                     measurement=measurement_params).to(self.device)
        else:
            out_features = feature_dims

        self.fc1 = nn.Linear(out_features, out_features).to(self.device)
        self.fc2 = nn.Linear(out_features, out_features // 2).to(self.device)
        self.fc3 = nn.Linear(out_features // 2, n_classes).to(self.device)
