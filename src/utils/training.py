...
class Trainer:
    ...
    def fit(self):
        ...
        if self.schedulefree:
            self.optimizer = sf.AdamWScheduleFree(model.parameters(), lr=lr, weight_decay= 1e-4)
        else:
        ...
        if self.plot:
                ...
                accuracies(self.train_accuracies, self.val_accuracies)
                losses(self.train_losses, self.val_losses)
                other_metrics(self.precision, self.recall, self.f1)
